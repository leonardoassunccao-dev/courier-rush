...
const SHADOW_CASTERS = new Set(['cabin','cargo','chassis','tank','rear-door']);
...
function cast(group) {
  group.traverse(child => { if (child.isMesh) { child.castShadow = SHADOW_CASTERS.has(child.name); child.receiveShadow = child.castShadow; child.updateMatrix(); child.matrixAutoUpdate = false; } });
  return group;
}
...
function createWheel(radius, width, colors, detailed) {
  ...
  if (detailed) {
    const treadGeometry=new THREE.BoxGeometry(width+.035,.105,radius*.42);const treads=new THREE.InstancedMesh(treadGeometry,material(0x080a0b,.96),10);const dummy=new THREE.Object3D();for(let treadIndex=0;treadIndex<10;treadIndex++){const angle=treadIndex/10*Math.PI*2;dummy.position.set(0,Math.cos(angle)*radius,Math.sin(angle)*radius);dummy.rotation.x=angle;dummy.updateMatrix();treads.setMatrixAt(treadIndex,dummy.matrix);}treads.instanceMatrix.setUsage(THREE.StaticDrawUsage);treads.name='treads';wheel.add(treads);
  }
  return wheel;
}
...
function buildCab(group, colors, premium, detailed) {
  ...
  [-1,1].forEach(side=>add(group,cube(.045,.72,1.02,glass),side*1.435,2.38,-2.62,'side-window'));
  add(group, cube(2.52, .6, .09, dark), 0, 1.38, -3.69, 'grille');
  if(detailed)for (let i = -2; i <= 2; i++) add(group, cube(.1, .42, .035, material(0x687173, .3, .7)), i * .42, 1.38, -3.75);
  ...
  if (detailed) {
    [-1, 1].forEach(side => {
      add(group,cube(.055,1.35,.035,dark),side*1.465,1.62,-2.28,'door-seam');add(group,cube(.06,.07,.32,material(0x687173,.3,.7)),side*1.47,1.92,-2.1,'door-handle');
...
function addBoxBody(group, colors, length, kind, startZ = .9, detailed = true) {
  ...
  [-1,1].forEach(side=>{add(group,cube(.055,.12,length-.22,material(0x778081,.34,.62)),side*1.535,1.08,startZ,'side-reflector');if(detailed)for(let z=startZ-length/2+.45;z<startZ+length/2;z+=1.15)add(group,cube(.06,2.62,.075,material(0x8c9290,.42,.42)),side*1.54,2.28,z,'cargo-rib');});
...
function addImplement(group, type, colors, detailed) {
  ...
  if (['box','sider','reefer','container'].includes(type)) {
    rearZ = addBoxBody(group, colors, 6, type === 'box' || type === 'container' ? 'long' : type, 4.5, detailed);
    ...
    for(let i=0;i<count;i++){const center=3.15+i*3.45;rearZ=addBoxBody(group,colors,length,i%2?'sider':'long',center,detailed);}
  }
...
export function createVehicle(vehicle = { body: 'urban' }, custom = {}, detailed = true) {
  ...
  if (vehicle.body.startsWith('tractor') || vehicle.body === 'premium') {
    ...
    addTractorRear(group, colors, premium, detailed);
    rearZ = addBoxBody(group, colors, 3.7, 'long', 3.9, detailed); addRearDetails(group, rearZ, colors);
    if (vehicle.body === 'rodotrem') { rearZ = addBoxBody(group, colors, 3.4, 'sider', 7.7, detailed); addRearDetails(group, rearZ, colors); }
  } else {
    const length = vehicle.body === 'urban' ? 5.0 : vehicle.body === 'long' || vehicle.body === 'reefer' ? 7.0 : 6.0;
    rearZ = addBoxBody(group, colors, length, vehicle.body, vehicle.body === 'urban' ? .6 : 1.6, detailed);
...
export function createTrafficTruck() {
  const group=new THREE.Group();const bodyColor=Math.random()>.5?0xcbd2d2:0x7f8a88;const bodyMat=material(bodyColor,.44,.18);const dark=material(0x172022,.4,.5);
  add(group,cube(2.72,2.62,4.8,bodyMat),0,2.2,.72,'cargo');add(group,cube(2.66,2.2,2.05,bodyMat),0,1.86,-2.45,'cabin');add(group,cube(2.18,.7,.055,material(0x1a2b31,.15,.5)),0,2.34,-3.5);add(group,cube(2.25,.3,6.2,dark),0,.66,.05,'chassis');
  const rearZ=3.15;add(group,cube(2.42,2.3,.055,material(0xd2d4d0,.5,.2)),0,2.2,rearZ,'rear-door');add(group,cube(2.72,.16,.07,material(0x7e242c,.4,.3)),0,1.08,rearZ+.06);
  const positions=[[-1.3,-2.2],[1.3,-2.2],[-1.3,2.05],[1.3,2.05]];const dummy=new THREE.Object3D();const tires=new THREE.InstancedMesh(new THREE.CylinderGeometry(.5,.5,.32,12),material(0x0c0f10,.9),4);const rims=new THREE.InstancedMesh(new THREE.CylinderGeometry(.23,.23,.345,12),material(0x969d9c,.25,.75),4);positions.forEach(([x,z],index)=>{dummy.position.set(x,.65,z);dummy.rotation.set(0,0,Math.PI/2);dummy.updateMatrix();tires.setMatrixAt(index,dummy.matrix);rims.setMatrixAt(index,dummy.matrix);});tires.instanceMatrix.setUsage(THREE.StaticDrawUsage);rims.instanceMatrix.setUsage(THREE.StaticDrawUsage);group.add(tires,rims);
  const tailMaterial=new THREE.MeshStandardMaterial({color:0x77050b,emissive:0xff1025,emissiveIntensity:2,toneMapped:false});const tails=new THREE.InstancedMesh(new THREE.BoxGeometry(.44,.16,.07),tailMaterial,2);[-.88,.88].forEach((x,index)=>{dummy.position.set(x,.96,rearZ+.09);dummy.rotation.set(0,0,0);dummy.updateMatrix();tails.setMatrixAt(index,dummy.matrix);});tails.instanceMatrix.setUsage(THREE.StaticDrawUsage);group.add(tails);group.userData.wheels=[];return cast(group);
}

export function createTrafficCar(color = 0x31586b) {
  const group = new THREE.Group(); const bodyMat = material(color, .32, .35);
  add(group, cube(2.15, .72, 4.1, bodyMat), 0, .75, 0, 'cabin');
  add(group, cube(1.75, .68, 2.1, bodyMat), 0, 1.42, -.15, 'cargo');
  add(group, cube(1.78, .42, .08, material(0x16262c, .18, .5)), 0, 1.53, -1.24);
  const positions=[[-1.03,-1.25],[1.03,-1.25],[-1.03,1.25],[1.03,1.25]];const dummy=new THREE.Object3D();const tires=new THREE.InstancedMesh(new THREE.CylinderGeometry(.34,.34,.22,12),material(0x0c0f10,.9),4);const rims=new THREE.InstancedMesh(new THREE.CylinderGeometry(.16,.16,.245,12),material(0x9ca2a0,.24,.82),4);positions.forEach(([x,z],index)=>{dummy.position.set(x,.48,z);dummy.rotation.set(0,0,Math.PI/2);dummy.updateMatrix();tires.setMatrixAt(index,dummy.matrix);rims.setMatrixAt(index,dummy.matrix);});tires.instanceMatrix.setUsage(THREE.StaticDrawUsage);rims.instanceMatrix.setUsage(THREE.StaticDrawUsage);group.add(tires,rims);
  group.userData.wheels = []; return cast(group);
}
